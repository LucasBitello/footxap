...
class ModelDataLTSM:
    def __init__(self, arrEntradas: list[list], arrRotulos: list[list[list]],
        ...
        self.taxa_regularização_l2: float = 0.00000000000001
    ...
    def backward(self, entradas: list, saidas: list, rotulos: list, celulas_rede: list, estados_ocultos_rede: list):
        ...
        for index_entrada_t in reversed(range(len(entradas))):
            ...
            for index_camada_saida in reversed(range(len(self.arrCamadaSaida))):
                ...
                dsaida = erro
                dot_dsaida = numpy.dot(dsaida, estado_oculto_ultima_camada_t.T)

                dWv[index_camada_saida] += dot_dsaida
                dbv[index_camada_saida] += erro

                dot_destado_oculto_next = numpy.dot(self.Wv[index_camada_saida].T, dsaida) * funcDerivadaOculta(estado_oculto_ultima_camada_t)
            ...
            for index_camada_oculta in reversed(range(len(self.arrNEstadosOcultos))):
                ...
                dc_ = numpy.clip((destado_oculto_next[index_camada_oculta] * o * \
                      self.modelDataLTSM.iaRegras.derivada_tanh(estado_celula)) + destado_celula_next[index_camada_oculta], -5, 5)
                ...
                df = dc_ * estado_celula_prev * self.modelDataLTSM.iaRegras.derivada_sigmoid(f)

                di = dc_ * c * self.modelDataLTSM.iaRegras.derivada_sigmoid(i)
