...
class DatasetPartidaEntrada:
    ...
    def obterDadoEntradaEmArray(self, isSomenteTeam: bool = True):
        if isSomenteTeam:
            arrDados = [self.id_team_home, self.season_pontos_home, self.season_saldo_gols_home,
                        ...
                        self.id_team_away, self.season_pontos_away, self.season_saldo_gols_away,
            ...
            arrDados = [self.id_team_home, self.season_pontos_home, self.season_saldo_gols_home,
                        self.team_pontos_home, self.team_saldo_gols_home, self.season_media_gols_marcados_home,
                        self.season_media_gols_sofridos_home,  self.team_media_gols_marcados_home,
                        self.team_media_gols_sofridos_home, self.team_media_resultados_partida_home,
                        self.team_melhorando_resultados_partida_home,

                        self.id_team_away, self.season_pontos_away, self.season_saldo_gols_away,
                        self.team_pontos_away,  self.team_saldo_gols_away, self.season_media_gols_marcados_away,
                        self.season_media_gols_sofridos_away, self.team_media_gols_marcados_away,
                        self.team_media_gols_sofridos_away, self.team_media_resultados_partida_away,
                        self.team_melhorando_resultados_partida_away]

        return arrDados

    def obterDadoRotulosEmArray(self, isPartida: bool = True, id_team: int = None):
        # cada array interno representa uma camada de saida.
        if isPartida:
            arrDados = [[self.dataset_partida_rotulo.chance_vitoria], [self.dataset_partida_rotulo.chance_empate],
                        [self.dataset_partida_rotulo.winner]]
        else:
            if id_team == self.id_team_home:
                arrDados = [[self.dataset_partida_rotulo.winner_home], [self.dataset_partida_rotulo.chance_empate]]
            elif id_team == self.id_team_away:
                arrDados = [[self.dataset_partida_rotulo.winner_away], [self.dataset_partida_rotulo.chance_empate]]
            else:
                arrDados = [[self.dataset_partida_rotulo.winner_home], [self.dataset_partida_rotulo.chance_empate]]
...
class DatasetPartidasRegras:
    def obter(self, arrIdsTeam: list[int], limitHistoricoMedias: int = 5, isNormalizarSaidaEmClasse: bool = True,
        ...
        for datasetPartida in arrDatasetPartida:
            ...
            arrDatasetEntradaEmArray.append(datasetPartida.obterDadoEntradaEmArray())
            arrDatasetRotuloEmArray.append(datasetPartida.obterDadoRotulosEmArray(isPartida=True, id_team=arrIdsTeam[0]))
        ...
        for indexDadoEntrada in range(len(arrDatasetEntradaEmArrayNormalizado)):
            ...
            for indexValue in range(len(arrDatasetEntradaEmArrayNormalizado[indexDadoEntrada])):
                if indexValue == 0 or indexValue == 7:
                    id_team = int(iaUteisRegras.desnormalizarValue(
            ...
            if isEncontrou:
                ...
                # arrDatasetEntradaEmArrayNormalizado[indexDadoEntrada].pop(1)
        ...
        return arrAAA[-15:], arrBBB[-15:], arrDatasetPreverEmArrayNormalizado
    ...
    def obterInfosDatabase(self, arrIdsTeam: list[int], limitHistoricoMedias):
        ...
        for fixture in arrFixtures:
            ...
            saldoGolsPartida = (newDatasetPartida.dataset_partida_rotulo.gols_home -
                                newDatasetPartida.dataset_partida_rotulo.gols_away)
    ...
    def obterUltimoDatasetPartida(self, arrDatasetPartida: list[DatasetPartidaEntrada],
                                  id_team: int, id_season: int = None) -> DatasetPartidaEntrada:
